    ...
    <main class="container-fluid px-4 mb-5" style="margin-top: 40px;">
        <div class="card shadow-sm border-0 rounded-4">
            <div class="card-header bg-white border-bottom-0 mt-3 pt-4 pb-3 d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="fw-bold text-primary"><i class="fas fa-list-alt me-2"></i>Lista de Todos os Formandos da Instituição</h3>
                    <p class="text-muted small mb-0">Faça a gestão dos formandos da instituição no sistema</p>
                </div>
                <div class="w-25">
            ...
            <div class="card-body p-4 pt-0">
                <div class="mb-3 d-flex gap-2">
                    <a href="/estagio/formando/criar" class="btn btn-primary shadow-sm"><i class="fas fa-plus-circle me-1"></i> Novo Formando</a>
    ...
    <script>
        $(document).ready(function() {
            ...
            function renderTable() {
                        ...
                        <tr>
                            <td colspan="15" class="text-center">
                                <div class="empty-state">
                                    ...
                                    <p class="mb-0">Nenhum formando encontrado</p>
                                    <small class="text-muted">Tente ajustar os filtros ou criar um novo formando</small>
                                </div>
                            </td>
                        </tr>
                    `);
                ...
                pageData.forEach(formando => {
                    $('#pedidosTbody').append(`
                        <tr>
                            <td><input type="checkbox" class="select-checkbox" value="${formando.id_formando}"></td>
                            <td>${formando.id_formando}</td>
                            <td>${formando.nome}</td>
                            <td>${formando.apelido}</td>
                            <td>${formando.codigo}</td>
                            <td>${formando.nascimento}</td>
                            <td>${formando.naturalidade}</td>
                            <td>${formando.tipoDocumento}</td>
                            <td>${formando.numeroDocumento}</td>
                            <td>${formando.localEmitido}</td>
                            <td>${formando.Emissao}</td>
                            <td>${formando.NUIT}</td>
                            <td>${formando.telefone}</td>
                            <td>${formando.email}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="btn btn-sm btn-warning editar-btn" data-id="${formando.id_formando}" title="Editar" >
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger remover-btn" data-id="${formando.id_formando}" title="Remover">
            ...
            function buscarPedidos(pesquisa = '') {
                $.ajax({
                    url: '/estagio/api/listar-formandos',
                    method: 'GET',
                    data: { termo: pesquisa },
                    dataType: 'json',
            ...
            $(document).on('click', '.editar-btn', function() {
                var id = $(this).data('id');
                window.location.href = '/estagio/formando/editar/' + id;
            });
