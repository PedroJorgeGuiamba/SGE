class SessionManager {
    ...
    init() {
        ...
        console.log(`SessionManager inicializado (timeout: ${this.config.timeoutMinutes}min, heartbeat: ${this.config.heartbeatInterval}s)`);
    }
    ...
    checkHeartbeat() {
        ...
        if (Date.now() - this.lastHeartbeat >= (this.config.heartbeatInterval * 1000)) {
            this.sendHeartbeat();
        }
    }
    ...
    checkInactivity() {
        ...
        else if (!this.warningShown && inactiveTime >= (timeoutMs - (this.config.warningMinutes * 60 * 1000))) {
    ...
    setupBeacon() {
        window.addEventListener('beforeunload', () => {
            ...
            fetch(this.config.heartbeatUrl, {
                method: 'POST',
                keepalive: true,
                headers: { 'X-Requested-With': 'beacon' }
            }).catch(() => {});
        });
