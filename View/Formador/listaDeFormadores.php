...
<main class="container-fluid px-4 mb-5" style="margin-top: 20px;">
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-header bg-white border-bottom-0 mt-3 pt-4 pb-3 d-flex justify-content-between align-items-center">
            <div>
                <h3 class="fw-bold text-primary"><i class="fas fa-list-alt me-2"></i>Lista de Todos os Formadores da Instituição</h3>
                <p class="text-muted small mb-0">Faça a gestão dos formadores da instituição no sistema</p>
            </div>
            <div class="w-25">
...
<script>
    $(document).ready(function() {
        ...
        function renderTable() {
                        ...
                        <tr>
                            <td colspan="15" class="text-center">
                                <div class="empty-state">
                                    ...
                                    <p class="mb-0">Nenhum formador encontrado</p>
                                    <small class="text-muted">Tente ajustar os filtros ou criar um novo formador</small>
                                </div>
                            </td>
                        </tr>
                    `);
            ...
            pageData.forEach(formador => {
                $('#pedidosTbody').append(`
                        <tr>
                            <td><input type="checkbox" class="select-checkbox" value="${formador.id_formador}"></td>
                            <td>${formador.id_formador}</td>
                            <td>${formador.nome}</td>
                            <td>${formador.apelido}</td>
                            <td>${formador.codigo}</td>
                            <td>${formador.profissao}</td>
                            <td>${formador.nascimento}</td>
                            <td>${formador.naturalidade}</td>
                            <td>${formador.tipoDocumento}</td>
                            <td>${formador.numeroDocumento}</td>
                            <td>${formador.localEmitido}</td>
                            <td>${formador.Emissao}</td>
                            <td>${formador.NUIT}</td>
                            <td>${formador.telefone || 'N/A'}</td>
                            <td>${formador.email}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="btn btn-sm btn-warning editar-btn" data-id="${formador.id_formador}" title="Editar" >
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger remover-btn" data-id="${formador.id_formador}" title="Remover">
        ...
        function buscarPedidos(pesquisa = '') {
            $.ajax({
                url: '/estagio/api/listar-formadores',
                method: 'GET',
                data: {
                    termo: pesquisa
                },
        ...
        $(document).on('click', '.editar-btn', function() {
            var id = $(this).data('id');
            window.location.href = '/estagio/formador/editar/' + id;
        });
