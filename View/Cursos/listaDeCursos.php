    ...
    <main class="container-fluid px-4 mb-5" style="margin-top: 40px;">
        <div class="card shadow-sm border-0 rounded-4">
            <div class="card-header bg-white border-bottom-0 mt-3 pt-4 pb-3 d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="fw-bold text-primary"><i class="fas fa-list-alt me-2"></i>Lista de Todos os Cursos da Instituição</h3>
                    <p class="text-muted small mb-0">Faça a gestão dos cursos da instituição no sistema</p>
                </div>
                <div class="w-25">
                    <div class="input-group">
                        <span class="input-group-text bg-light border-end-0"><i class="fas fa-search text-muted"></i></span>
                        <input type="text" id="searchInput" class="form-control border-start-0 bg-light" placeholder="Pesquisar por Nome, Descricao, Sigla...">
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4 pt-0">
                <div class="mb-3 d-flex gap-2">
                    <a href="/estagio/cursos/criar" class="btn btn-primary shadow-sm"><i class="fas fa-plus-circle me-1"></i> Novo Curso</a>
        ...
        $(document).ready(function() {
            ...
            function renderTable() {
                        ...
                        <tr>
                            <td colspan="15" class="text-center">
                                <div class="empty-state">
                                    ...
                                    <small class="text-muted">Tente ajustar os filtros ou criar um novo curso</small>
                                </div>
                            </td>
                        </tr>
                    `);
                ...
                pageData.forEach(curso => {
                    $('#pedidosTbody').append(`
                        <tr>
                            <td><input type="checkbox" class="select-checkbox" value="${curso.id_curso}"></td>
                            <td>${curso.id_curso}</td>
                            <td>${curso.codigo}</td>
                            <td>${curso.nome}</td>
                            <td>${curso.descricao}</td>
                            <td>${curso.sigla}</td>
                            <td>${curso.qualificacao_descricao ?? curso.qualificacao}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="btn btn-sm btn-warning editar-btn" data-id="${curso.id_curso}" title="Editar" >
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger remover-btn" data-id="${curso.id_curso}" title="Remover">
            ...
            $(document).on('click', '.editar-btn', function() {
                var id = $(this).data('id');
                window.location.href = '/estagio/cursos/editar/' + id;
            });
