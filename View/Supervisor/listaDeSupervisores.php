    ...
    <script>
        $(document).ready(function() {
            ...
            function renderTable() {
                        ...
                        <tr>
                            <td colspan="15" class="text-center">
                                <div class="empty-state">
                                    ...
                                    <p class="mb-0">Nenhum supervisor encontrado</p>
                                    <small class="text-muted">Tente ajustar os filtros ou criar um novo supervisor</small>
                                </div>
                            </td>
                        </tr>
                    `);
                ...
                pageData.forEach(supervisor => {
                    $('#pedidosTbody').append(`
                        <tr>
                            <td><input type="checkbox" class="select-checkbox" value="${supervisor.id_supervisor}"></td>
                            <td>${supervisor.id_supervisor}</td>
                            <td>${supervisor.nome}</td>
                            <td>${supervisor.qualificacao}</td>
                            <td>${supervisor.area}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="btn btn-sm btn-warning editar-btn" data-id="${supervisor.id_supervisor}" title="Editar" >
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger remover-btn" data-id="${supervisor.id_supervisor}" title="Remover">
            ...
            function buscarPedidos(pesquisa = '') {
                $.ajax({
                    url: '/estagio/api/listar-supervisores',
                    method: 'GET',
                    data: { termo: pesquisa },
                    dataType: 'json',
            ...
            $(document).on('click', '.editar-btn', function() {
                var id = $(this).data('id');
                window.location.href = '/estagio/supervisor/editar/' + id;
            });
