        ...
        $(document).ready(function() {
            ...
            function renderTable() {
                ...
                pageData.forEach(resposta => {
                    ...
                    const numeroPedido = resposta.numero_pedido ?
                        resposta.numero_pedido : ('ID ' + (resposta.numero_carta || 'N/A'));
                    const avaliarButton = resposta.status_estagio === 'Concluido' && resposta.status_resposta === 'Aceito'?
                        `<button class="btn btn-sm btn-primary avaliar-btn" data-id="${resposta.id_resposta}" title="Avaliar EstÃ¡gio">
            ...
            function buscarRespostas(pesquisa = '') {
                $.get('../../Controller/Estagio/search_respostas.php', { numero: pesquisa }, function(data) {
                    ...
                    console.error('Erro na requisiÃ§Ã£o:', status, error);
                    respostasData = [];
                    renderTable();
                });
            }
            ...
            $('#deleteSelected').on('click', function() {
                ...
                if (confirm('Tem certeza que deseja remover as respostas selecionadas?')) {
                    $.ajax({
                        url: '../../Controller/Estagio/remover_resposta.php', // Nota: Pode precisar ajustar o backend para aceitar mÃºltiplos IDs (ex: { numeros: selectedIds })
                        type: 'POST',
            ...
            $(document).on('click', '.remover-btn', function() {
                ...
                if (confirm('Tem certeza que deseja remover a resposta nÂº ' + id + '?')) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                alert(response.message);
                                // Atualiza a tabela apÃ³s remoÃ§Ã£o
                                buscarRespostas($('#searchInput').val().trim());
                            } else {
